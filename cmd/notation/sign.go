...
func signCommand() *cobra.Command {
	...
	cmd.SetPflagKey(command)
	cmd.SetPflagKeyFile(command)
	cmd.SetPflagCertFile(command)
	cmd.SetPflagTimestamp(command)
	cmd.SetPflagExpiry(command)
	cmd.SetPflagReference(command)
	...
	cmd.SetPflagPluginConfig(command)
	return command
}
...
func prepareSigningContent(command *cobra.Command) (notation.Descriptor, notation.SignOptions, error) {
	...
	if identity, _ := command.Flags().GetString(cmd.PflagReference.Name); identity != "" {
		manifestDesc.Annotations = map[string]string{
			"identity": identity,
		}
	}
	var tsa timestamp.Timestamper
	if endpoint, _ := command.Flags().GetString(cmd.PflagTimestamp.Name); endpoint != "" {
		if tsa, err = timestamp.NewHTTPTimestamper(nil, endpoint); err != nil {
			return notation.Descriptor{}, notation.SignOptions{}, err
		}
	}
	pluginConfig, err := cmd.ParsePflagPluginConfig(command)
	if err != nil {
		return notation.Descriptor{}, notation.SignOptions{}, err
	}
