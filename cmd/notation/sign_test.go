...
func TestSignCommand(t *testing.T) {
	...
	if key, _ := command.Flags().GetString(cmd.PflagKey.Name); key != "kkk" {
		t.Fatalf("Expect %v: %v, got: %v", cmd.PflagKey.Name, "kkk", key)
	}
...
func TestSignWithConfig(t *testing.T) {
	...
	config, err := cmd.ParsePflagPluginConfig(command)
	if err != nil {
